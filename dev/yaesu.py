...
class RIG(serial.Serial, object):
    ...
    ## >>>>>>>>>>>>>>>>>>>>>>>> Radio Function <<<<<<<<<<<<<<<<<<<<<<<<
    def cmd_ret_check(self, syntax, ret):
        pass

    def cmd_exec(self, func, *args):
        func_type = func.split('_')[-1]
        if func_type == 'get':
            cmd_ret = self.send(self.profile[func]['cmd'])
            ret = []
            for i in range(1, int(self.profile[func]['p_num'])+1):
                seg_p_name = 'p' + str(i)
                seg_d_name = 'dim' + str(i)
                seg_pos = self.profile[func][seg_p_name].split('|')
                seg_raw = cmd_ret[seg_pos[0], seg_pos[1]]
                if self.profile[func][seg_p_name].has_key(seg_d_name):
                    ret.append(self.profile[func][seg_p_name][seg_d_name][seg_raw])
                else:
                    ret.append(seg_raw)
        elif func_type == 'set':
            cmd_mod = self.profile[func]['cmd']
            seg_d_name = 'dim' + str(i)
            for i in range(1, int(self.profile[func]['p_num'])+1):
                seg_p_var = '{$p' + str(i) + '}'
                cmd_mod.replace(seg_p_var, args[i-1])
            self.send(cmd_mod)
    
    def af_gain_new(self, val=None):
        if val is None:
            return self.cmd_exec('af_gain_get',)
        else:
            return self.cmd_exec('af_gain_set', str(val).rjust(3,'0'))

    def att_new(self, stat=None):
        return 
